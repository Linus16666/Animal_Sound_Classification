...
/*
 * concatenation.cpp — sketch-folder override of the TFLite Micro concatenation
 * kernel.
 *
 * WHY THIS FILE EXISTS
 * The precompiled libespressif__esp-tflite-micro.a has kMaxInputNum=10 baked
 * in.  The unrolled LSTM in crnn.tflite produces CONCATENATION nodes with 107
 * inputs, so Prepare() fails at runtime with "kMaxInputNum was not true".
 *
 * HOW THE OVERRIDE WORKS
 * - Internal functions (Init/Prepare/Eval) are in an anonymous namespace
 *   (internal linkage) → no symbol conflict with the archive at all.
 * - Register_CONCATENATION() is defined in namespace tflite (external linkage)
 *   → the linker uses THIS definition (sketch objects link before archives)
 *   and the archive version is never pulled in.
 * - AddConcatenation() in MicroMutableOpResolver calls tflite::Register_CONCATENATION()
 *   which now registers OUR Prepare() that allows up to 110 inputs.
 *
 * ONLY CHANGE vs. upstream source:
 *   constexpr int kMaxInputNum = 110;   // was 10
 */
...
namespace {  // anonymous — internal linkage, no conflict with archive symbols

constexpr int kMaxInputNum = 110;  // raised from 10; crnn.tflite needs 107
...
inline void GetAllInputTensorShapes(const TfLiteContext* context,
  ...
  for (int i = 0; i < node->inputs->size; ++i) {
    const TfLiteEvalTensor* t = micro::GetEvalInput(context, node, i);
    RuntimeShape shape = micro::GetTensorShape(t);
...
inline void GetAllInputTensorData(const TfLiteContext* context,
  ...
  for (int i = 0; i < node->inputs->size; ++i) {
    const TfLiteEvalTensor* t = micro::GetEvalInput(context, node, i);
    all_data[i] = micro::GetTensorData<T>(t);
  }
}
...
void EvalUnquantized(TfLiteContext* context, TfLiteNode* node) {
  ...
  reference_ops::Concatenation(data->params, inputs_shape_ptr, inputs_data,
                               ...
                               micro::GetTensorData<data_type>(output));
}
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE(context, num_inputs <= kMaxInputNum);  // 110 — covers 107
...
// Defined in namespace tflite (not tflite::ops::micro) to match the
// declaration in micro_mutable_op_resolver.h and concatenation.h.
TFLMRegistration Register_CONCATENATION() {
  return micro::RegisterOp(Init, Prepare, Eval);
}
